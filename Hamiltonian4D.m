...
H = @(pu,pv,px,py,thu,thv,thx,thy) bu(pu,pv,px,py).*thu + bv(pu,pv,px,py).*thv + ...
    bx(pu,pv,px,py).*thx + by(pu,pv,px,py).*thy + 0.5*(thv.^2 + thy.^2);
...
dH_dthetav = @(pu,pv,px,py,thv) bv(pu,pv,px,py) + thv;
dH_dthetax = @(pu,pv,px,py) bx(pu,pv,px,py);
dH_dthetay = @(pu,pv,px,py,thy) by(pu,pv,px,py) + thy;
...
[x_,u_] = meshgrid(linspace(-1.5,1.5,100), linspace(-2,2,100));
[x_,y_] = meshgrid(linspace(-1.5,1.5,100), linspace(-2,2,100));

%total time should remain small
Nt = 100;
T = 15;
t = linspace(0,T,Nt);
dt = t(2)-t(1);
...
for outer=1:100
  for iter=1:1e6
    ...
    %if iter == 25
    % gradcomparey = zeros(1,Nt-1);
    % gradcomparev = zeros(1,Nt-1);
    % gradcompare = zeros(1,Nt-1);
    % h = 1e-7;
    % for i = 1:Nt-1
    %     vector = zeros(1,Nt-1);
    %     vector(i) = 1;
    %     costA = cost(thv,thy,params);
    %     costB = cost(thv,thy+h*vector,params);
    %     gradcomparey(i) = (costB-costA)/h;
    %     costC = cost(thv+h*vector,thy,params);
    %     gradcomparev(i) = (costC-costA)/h;
         
    % end
    % figure(3)
    % plot(grady,'x-'); hold on
    % plot(gradcomparey,'g-'); hold off
    % figure(4)
    % plot(gradv,'x-'); hold on
    % plot(gradcomparev,'g-'); hold off
    % end
	...
	% if mod(iter,25)==0
     ...
     %  pv(s) = pv(s-1) + params.dt*params.dH_dthetav(pu(s-1),pv(s-1),px(s-1),py(s-1),thv(s-1));
	 %  px(s) = px(s-1) + params.dt*params.dH_dthetax(pu(s-1),pv(s-1),px(s-1),py(s-1));
	 %  py(s) = py(s-1) + params.dt*params.dH_dthetay(pu(s-1),pv(s-1),px(s-1),py(s-1),thy(s-1));
     % end
     % figure(1)
	 % contourf(x_,u_,V(x_,u_), 13); hold on;
     % %plot(pu,pv,'r-x'); 
     % plot(pu,px,'g-x'); hold off
...
function ret=cost(thv,thy,params)
  ...
  ret = 0.5*sum((thv.^2+thy.^2)*params.dt) ...
...
function [ret1,ret2]=gradient(thv,thy,params)
  ...
  ret1 = (thv - muv)*params.dt;
  ret2 = (thy - muy)*params.dt; % gradient computation
end
